...
class RequestBodyBuilder : Builder<RequestBody?> {
    ...
    fun setRequestBody4Text(textStr: String) {
        requestBody = RequestBody.create(MEDIA_TYPE_PLAIN.toMediaTypeOrNull(), textStr)
    }

    fun setRequestBody4JSon(jsonStr: String) {
        requestBody = RequestBody.create(MEDIA_TYPE_JSON.toMediaTypeOrNull(), jsonStr)
    }

    fun setRequestBody4Xml(xmlStr: String) {
        requestBody = RequestBody.create(MEDIA_TYPE_XML.toMediaTypeOrNull(), xmlStr)
    }

    fun setRequestBody4Byte(bytes: ByteArray) {
        requestBody = RequestBody.create(MEDIA_TYPE_STREAM.toMediaTypeOrNull(), bytes)
    }
