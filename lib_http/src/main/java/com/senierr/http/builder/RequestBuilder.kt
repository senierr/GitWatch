...
import com.senierr.http.executor.RealExecutor
...
class RequestBuilder(
    ...
    var uploadListener: ((totalSize: Long, currentSize: Long, percent: Int) -> Unit)? = null
    var downloadListener: ((totalSize: Long, currentSize: Long, percent: Int) -> Unit)? = null
     ...
     * 上传标签
     */
    fun onUpload(uploadListener: (totalSize: Long, currentSize: Long, percent: Int) -> Unit): RequestBuilder {
     ...
     * 下载标签
     */
    fun onDownload(downloadListener: (totalSize: Long, currentSize: Long, percent: Int) -> Unit): RequestBuilder {
    ...
    fun execute(): Response {
        return RealExecutor(okHttpClient, build()).execute()
    }

    /**
     * 挂起函数
     */
    suspend fun <T> execute(converter: Converter<T>): T {
        return RealExecutor(okHttpClient, build()).execute(
            converter,
            uploadListener,
            downloadListener
